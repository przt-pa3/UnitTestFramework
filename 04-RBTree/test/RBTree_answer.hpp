...
void RBTree<T>::remove_max() {
    ...
    if (root.get()->left && root.get()->right &&
        !RBNode<T>::is_red(root.get()->left) && !RBNode<T>::is_red(root.get()->right))
        root->color = RED;
...
RBNode<T>* RBNode<T>::rotate_right(std::unique_ptr<RBNode<T>>& n) {
    // TODO
    RBNode<T>* thisnode = n.get();
    std::unique_ptr<RBNode<T>> leftkid = nullptr;

    leftkid = std::move(thisnode->left);
    thisnode->left = std::move(leftkid.get()->right);

    leftkid.get()->right = std::move(n);

    leftkid.get()->color = leftkid.get()->right.get()->color;
    leftkid.get()->right.get()->color = RED;

    return leftkid.release();
}
...
RBNode<T>* RBNode<T>::rotate_left(std::unique_ptr<RBNode<T>>& n) {
    // TODO
    RBNode<T>* thisnode = n.get();
    std::unique_ptr<RBNode<T>> rightkid = nullptr;

    rightkid = std::move(thisnode->right);
    thisnode->right = std::move(rightkid.get()->left);

    rightkid.get()->left = std::move(n);

    rightkid.get()->color = rightkid.get()->left.get()->color;
    rightkid.get()->left.get()->color = RED;

    return rightkid.release();
}
...
RBNode<T>* RBNode<T>::insert(std::unique_ptr<RBNode<T>>& n, const T& t) {
    ...
    // insert at bottom
    if (n == nullptr) {
        std::unique_ptr<RBNode<T>> newnode = std::make_unique<RBNode<T>>(t);
        n = std::move(newnode);
        return n.release();
    }
    ...
    //		RBNode<T>::is_red(thisnode->left) && RBNode<T>::is_red(thisnode->right))
    //	thisnode->flip_color();
    ...
    else if (n && t > thisnode->key) {
        RBNode<T>* kid = RBNode<T>::insert(thisnode->right, t);
        thisnode->right.reset(kid);
    }
    else {
        RBNode<T>* kid = RBNode<T>::insert(thisnode->left, t);
        thisnode->left.reset(kid);
    }

    // fix right-leaning reds ~ on the way up
    if (thisnode->right && RBNode<T>::is_red(thisnode->right))
        n.reset(RBNode<T>::rotate_left(n));

    thisnode = n.get();

    // fix two reds in a row ~ on the way up
    if (thisnode->left && thisnode->left.get()->left &&
        RBNode<T>::is_red(thisnode->left) && RBNode<T>::is_red(thisnode->left.get()->left))
        n.reset(RBNode<T>::rotate_right(n));
...
RBNode<T>* RBNode<T>::fix_up(std::unique_ptr<RBNode<T>>& n) {
    ...
    // rotate-left : right-leaning reds (fix back)
    if (thisnode->right && RBNode<T>::is_red(thisnode->right))
        n.reset(RBNode<T>::rotate_left(n));
...
RBNode<T>* RBNode<T>::move_red_right(std::unique_ptr<RBNode<T>>& n) {
    ...
    thisnode->flip_color();
    if (thisnode->left && thisnode->left.get()->left &&
        RBNode<T>::is_red(thisnode->left.get()->left)) {
...
RBNode<T>* RBNode<T>::remove_max(std::unique_ptr<RBNode<T>>& n) {
    ...
    RBNode<T>* thisnode = n.get();
    if (thisnode->left && RBNode<T>::is_red(thisnode->left))
        n.reset(RBNode<T>::rotate_right(n));

    thisnode = n.get();

    // removing node on bottom level
    if (!thisnode->right && !thisnode->left) {
        return nullptr;
    }

    if (!thisnode->right && thisnode->left) {
        n = std::move(thisnode->left);
        return n.release();
    }

    // i'm not sure with first condition
    if (thisnode->right.get()->left && !RBNode<T>::is_red(thisnode->right) &&
        !RBNode<T>::is_red(thisnode->right.get()->left))
        n.reset(RBNode<T>::move_red_right(n));

    thisnode = n.get();

    thisnode->right.reset(RBNode<T>::remove_max(thisnode->right));

    return RBNode<T>::fix_up(n);
}
...
RBNode<T>* RBNode<T>::move_red_left(std::unique_ptr<RBNode<T>>& n) {
    ...
    if (thisnode->right && thisnode->right.get()->left &&
        RBNode<T>::is_red(thisnode->right.get()->left)) {
        thisnode->right.reset(RBNode<T>::rotate_right(thisnode->right));
        n.reset(RBNode<T>::rotate_left(n));
        thisnode = n.get();
        thisnode->flip_color();
    }
...
RBNode<T>* RBNode<T>::remove_min(std::unique_ptr<RBNode<T>>& n) {
    ...
    if (!thisnode->left && thisnode->right) {
        n = std::move(thisnode->right);
        return n.release();
    }

    // Borrow from sibling
    if ((!thisnode->left.get()->left || !RBNode<T>::is_red(thisnode->left.get()->left)) &&
        !RBNode<T>::is_red(thisnode->left))
        n.reset(RBNode<T>::move_red_left(n));

    thisnode = n.get();
    thisnode->left.reset(RBNode<T>::remove_min(thisnode->left));
...
RBNode<T>* RBNode<T>::remove(std::unique_ptr<RBNode<T>>& n, const T& t) {
    ...
    if (n && t < thisnode->key) {
        if ((!thisnode->left.get()->left || !RBNode<T>::is_red(thisnode->left.get()->left)) &&
            ...
            n.reset(RBNode<T>::move_red_left(n));
        thisnode = n.get();
        thisnode->left.reset(RBNode<T>::remove(thisnode->left, t));
    }
    else {
        if (thisnode->left && RBNode<T>::is_red(thisnode->left))
            n.reset(RBNode<T>::rotate_right(n));
        ...
        if (n && t == thisnode->key && !thisnode->right && !thisnode->left)
            return nullptr;
        ...
        if (thisnode->right &&
            (!thisnode->right.get()->left ||
                !RBNode<T>::is_red(thisnode->right.get()->left)) &&
            !RBNode<T>::is_red(thisnode->right))
            n.reset(RBNode<T>::move_red_right(n));
        thisnode = n.get();

        if (n && t == thisnode->key) {
            thisnode->key = thisnode->right.get()->leftmost_key();
            thisnode->right.reset(RBNode<T>::remove_min(thisnode->right));
        }
        else
            thisnode->right.reset(RBNode<T>::remove(thisnode->right, t));
    }
    return RBNode<T>::fix_up(n);
}
